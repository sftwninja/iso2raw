...
impl IsoReader {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        if file_size % ISO_SECTOR_SIZE != 0 {
            anyhow::bail!("Invalid ISO file size: {} is not a multiple of {}", file_size, ISO_SECTOR_SIZE);
        }
...
impl RawWriter {
    ...
    pub fn write_sector(&mut self, data: &[u8]) -> Result<()> {
        if data.len() != RAW_SECTOR_SIZE {
            anyhow::bail!("Invalid RAW sector size: expected {}, got {}", RAW_SECTOR_SIZE, data.len());
        }

use iso2raw::converter::{Mode1Sector, RAW_SECTOR_SIZE, SYNC_PATTERN};
...
fn test_edc_calculation_properties() {
    ...
    assert_ne!(
        edc1, edc2,
        "Different data should produce different EDC values"
    );
    ...
    assert_eq!(
        edc_first, edc_second,
        "EDC calculation should be deterministic"
    );
}
...
fn test_complete_sector_generation() {
    ...
    assert!(
        !ecc_p.iter().all(|&b| b == 0),
        "P parity should not be all zeros"
    );
    assert!(
        !ecc_q.iter().all(|&b| b == 0),
        "Q parity should not be all zeros"
    );
}
